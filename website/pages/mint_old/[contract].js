...
export default function Home() {
  ...
  useEffect(() => {
    if (drops && drops.length > 0 && contract) {
      const drop = _.find(drops, { contract });
      setCollection(drop);
    }
  }, [contract, drops]);
      ...
      {collection && collection.contract && contract && (
        <div className="form-wrapper">
          ...
          {saleActive ? (
                  ...
                  {preSaleActive && nftsOwned == 0 ? (
                    ...
                    <div>
                      <div>
                        ...
                        <Button size="lg" variant="primary" onClick={() => mintNFTs()}>
                          Mint
                        </Button>
                        <div className="supply">
                          Total Minted: {totalMinted} / {supply}
                        </div>
                      </div>
                      <div>
                        <p>
                          <a
                            target="_blank"
                            href={`${etherscanUrl}/address/${collection.contract}`}
                            rel="noreferrer"
                          >
                            View Contract
                          </a>
                          <br />
                          {transactionUrl && (
                            <a target="_blank" href={transactionUrl} rel="noreferrer">
                              View Transaction
                            </a>
                          )}
                        </p>
            ...
            <div>
              {isMobile && saleActive ? (
                <Button size="lg" variant="primary" href={metamaskDeepLinkUrl}>
                  Connect
                </Button>
              ) : (
