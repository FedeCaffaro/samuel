...
export default function Home() {
  ...
  const approve = async () => {
    const result = await setApproveForAll(drops[0].contract, wallet.account);
    if (result.error) {
      setErrorShow(true);
      setError(result.error);
    } else {
  ...
  const selectStake = (asset) => {
    ...
    if (!_.find(stakes, { token_id: asset.token_id }) && stakes.length < 20) {
      ...
      setStakes(_.filter(stakes, (staked) => staked.token_id != asset.token_id));
    }
  };
  ...
  const loadAssets = async (drop) => {
    ...
    const getStakedAssets = async (tokenIds, offset, limit) => {
      if (tokenIds.length > 0) {
        ...
        const response = await fetch(
          `${OS_API_ENDPOINT}/assets?order_direction=asc&asset_contract_address=${drop.contract}${tokenString}`
        );
    ...
    const unstakedData = await getUnstakedAssets(wallet.account, drop.contract);
    const assets = [];
    ...
    <div className="den-wrapper">
                    ...
                    {wallet && wallet.account ? (
                      <div className="wallet-address">
                        <Button variant="outline-light" size="sm" onClick={() => wallet.reset()}>
                          ...
                          {`${wallet.account.substr(0, 6)}...${wallet.account.substr(
                            wallet.account.length - 4
                          )}`}
                        </Button>
                      </div>
                    ) : (
                      <Button variant="outline-light" size="sm" onClick={() => wallet.connect()}>
                        Connect Wallet
                      </Button>
                    )}
                  </div>
                ...
                <Tab.Content className="outter-tab-content">
                    ...
                    <Tabs defaultActiveKey="home-inner" className="inner-tabs">
                      <Tab eventKey="home-inner" title="Home" className="inner-tab-content home-content">
                        <div className="home-video">
                          ...
                          <div className="connect-wrapper">
                            ...
                            {!wallet ||
                              (!wallet.account && (
                                <Button className="btn-xl" onClick={() => wallet.connect()}>
                                  CONNECT WALLET
                                </Button>
                              ))}
                            <div className="connect-links">
                              <Button target="_blank" href="https://twitter.com/samotclub" variant="link">
                                <FaTwitter /> Twitter <AiOutlineDoubleRight />
                              </Button>
                              <Button
                                target="_blank"
                                href="https://snapshot.org/#/samotclub.eth"
                                variant="link"
                              >
                                DAO <AiOutlineDoubleRight />
                              </Button>
                              <Button target="_blank" href="https://discord.gg/E5ejHEwrwt" variant="link">
                                <FaDiscord /> Discord <AiOutlineDoubleRight />
                              </Button>
                            </div>
                          </div>
                        </div>
                      ...
                      <Tab eventKey="overview" title="Overview" className="inner-tab-content overview">
                        ...
                        <div className="overview-header">
                          <h1>Samot Club</h1>
                          <p>
                            An NFT Club that introduces the amazing Latin American artistic style. From
                            diverse artists, the first collection comes from the creator himself. Welcome to
                            Samot´s World and enjoy the ride.
                          </p>
                        </div>
                        <Row>
                  ...
                  <Tab.Pane eventKey="portfolio">
                    ...
                    >
                      {drops &&
                        drops.length > 0 &&
                        _.filter(drops, (drop) => drop.type == 'active' || drop.type == 'previous').map(
                          (drop) => (
                            <Dropdown.Item
                              key={drop.symbol}
                              onClick={() => loadAssets(drop)}
                              eventKey={drop.contract}
                            >
                              {drop.name}
                            </Dropdown.Item>
                          )
                    ...
                    {collection && collection.name == 'SAMOT' ? (
                      <div className="token-count">
                        <span>{unstakedAssets && unstakedAssets.length ? unstakedAssets.length : 0}</span>{' '}
                        unstaked |{' '}
                        <span>{stakedAssets && stakedAssets.length ? stakedAssets.length : 0}</span> staked |{' '}
                        <Button variant="primary" onClick={() => claimTotalRewards()}>
                          Claim Tokens
                        </Button>
                      </div>
                    ) : (
                    ...
                    {collection && collection.name == 'SAMOT' ? (
                      <div>
                        <Tabs
                          defaultActiveKey="unstaked"
                          className="inner-tabs portfolio"
                          onSelect={(k) => setTab(k)}
                          activeKey={tab}
                        >
                          <Tab eventKey="unstaked" title="Unstaked" className="inner-tab-content">
                            <Stats
                              ...
                              stakesCount={stakedAssets.length}
                            ...
                            <div className="nft-list">
                              {unstakedAssets && unstakedAssets.length < 1 && (
                                <div className="no-nfts">
                                  <span>
                                    No unstaked found in your wallet.
                                    <br />
                              ...
                              {unstakedAssets &&
                                ...
                                unstakedAssets.map((asset) => (
                                  <div
                                    className={
                                      _.find(stakes, { token_id: asset.token_id })
                                        ? 'nft-card selected'
                                        : 'nft-card'
                                    }
                                    key={`unstaked-${asset.token_id}`}
                                  >
                                    <Dropdown align="right" className="nft-menu">
                                      ...
                                      <Dropdown.Menu align="right">
                                        <Dropdown.Item onClick={() => selectStake(asset)}>
                                          Stake NFT
                                        </Dropdown.Item>
                                      </Dropdown.Menu>
                                    </Dropdown>
                              ...
                              <div className="nfts-selected-wrapper">
                                ...
                                <div className="selected-actions">
                                  {isApproved ? (
                                    ...
                                    <Button variant="outline-light" onClick={() => approve()}>
                                      Approve
                                    </Button>
                                  )}
                                  <Button variant="outline-light" onClick={() => unselectStake()}>
                                    Cancel
                                  </Button>
                                </div>
                              </div>
                          ...
                          >
                            <Stats
                              stakingRewards={stakingRewards}
                              stakesCount={stakedAssetsV1.length}
                              balanceTokens={balanceTokens}
                              percentageStaked={percentageStaked}
                            />
                            <div className="nft-list">
                              {stakedAssetsV1 && stakedAssetsV1.length < 1 && (
                                <div className="no-nfts">
                                  <span>
                                    No staked found in your wallet.
                                    <br />
                                    Make sure you are connected to mainnet.
                                  </span>
                                </div>
                              ...
                              {stakedAssetsV1 &&
                                stakedAssetsV1.length > 0 &&
                                stakedAssetsV1.map((asset) => (
                                  <div
                                    className={
                                      _.find(stakes, { token_id: asset.token_id })
                                        ? 'nft-card locked selected'
                                        : 'nft-card locked'
                                    }
                                    key={`staked-${asset.token_id}`}
                                  >
                                    <Dropdown align="right" className="nft-menu">
                                      <Dropdown.Toggle variant="outline-light" id="dropdown-basic">
                                        <span>
                                    ...
                                    <div className="lockedup">
                                      <img onClick={() => select(asset)} src="lock.png" width="120px" />
                                    </div>
                            ...
                            {staking && stakes && stakes.length > 0 && !transactionUrl && (
                              <div className="nfts-selected-wrapper">
                                ...
                                <div className="selected-actions">
                                  <Button variant="primary" onClick={() => unstakeV1()}>
                                    Unstake
                                  </Button>
                                  <Button variant="outline-light" onClick={() => unselectStake()}>
                                    Cancel
                            ...
                            {transactionUrl && (
                              <div className="transaction">
                                <Button target="_blank" variant="primary" size="lg" href={transactionUrl}>
                                  View Transaction
                                </Button>
                              </div>
                            )}
                          ...
                          >
                            <Stats
                              stakingRewards={stakingRewards}
                              stakesCount={stakedAssetsV2.length}
                              balanceTokens={balanceTokens}
                              percentageStaked={percentageStaked}
                            />
                            <div className="nft-list">
                              ...
                              {stakedAssetsV2 &&
                                stakedAssetsV2.length > 0 &&
                                stakedAssetsV2.map((asset) => (
                                  <div
                                    className={
                                      _.find(stakes, { token_id: asset.token_id })
                                        ? 'nft-card locked selected'
                            ...
                            {staking && stakes && stakes.length > 0 && !transactionUrl && (
                              <div className="nfts-selected-wrapper">
                                ...
                                <div className="selected-actions">
                                  <Button variant="primary" onClick={() => unstake()}>
                                    Unstake
                                  </Button>
                                  <Button variant="outline-light" onClick={() => unselectStake()}>
                                    Cancel
                          ...
                          {drops &&
                            _.filter(drops, { type: 'active' }).map((drop) => (
                              <Row className="vertical-center drop-row" key={`key-${drop.imageUrl}-drop`}>
                                ...
                                <Col sm={12} md={2} lg={2} xl={2}>
                                  <Button size="lg" onClick={() => showCollection(drop)}>
                                    View Collection
                                  </Button>
                                </Col>
                              </Row>
                    ...
                    <Tabs defaultActiveKey="token-inner" className="inner-tabs">
                      <Tab eventKey="token-inner" title="Overview" className="inner-tab-content">
                        ...
                        <div className="overview-header">
                          <h1>$AMOT Token</h1>
                          <p>
                            $AMOT is a mintable ERC20 token that can be burned to mint NFTs in Samot Club
                            drops, acquire merch, physical art, music and more. Details and tokenomics will be
                            released on this page soon.
                          </p>
                        </div>
                        {/* <Row>
                        <Col>
                          ...
                          <Col sm={12}>
                            <div className="utility-item">
                              <div>
                                <h4>NFTs</h4>
                                <p>
                                  If you're a Samot NFT staker, you will earn $AMOT tokens and you will have
                                  pre-sale access to Samot Club NFT drops. If you choose to participate in a
                                  drop, you can burn $AMOT tokens to mint NFTs in the pre-sale.
                                </p>
                              </div>
                            </div>
                          </Col>
                          <Col sm={12}>
                            <div className="utility-item">
                              <div>
                                <h4>Merch</h4>
                                <p>
                                  $AMOT tokens can be used to purchase merchandise from our upcoming store,
                                  which will feature a clothing line designed by Samot himself and digital
                                  merch that can be used in the metaverse.
                                </p>
                              </div>
                            </div>
                          </Col>
                          <Col sm={12}>
                            <div className="utility-item">
                              <div>
                                <h4>Gallery</h4>
                                <p>
                                  $SAMOT tokens can be used to purchase physical and digital art pieces from
                                  our upcoming online gallery, which will include art from some of the most
                                  well known artists in Latin America and around the world.
                                </p>
                              </div>
                            </div>
                          </Col>
                          <Col sm={12}>
                            <div className="utility-item">
                              <div>
                                ...
                                <p>
                                  If you're a Samot NFT staker, you will earn $AMOT tokens and you will have
                                  pre-sale access to Samot Club music NFT drops. If you choose to participate
                                  in a drop, you can burn $AMOT tokens to mint music NFTs in the pre-sale.
                                </p>
                              </div>
                            </div>
                          </Col>
                          <Col sm={12}>
                            <div className="utility-item">
                              <div>
                                <h4>Liquidity</h4>
                                <p>
                                  Samot Club members will be able to provide liquidity to the Samot Club
                                  community (similar to NFT20.io) and to the DEXs (e.g., Uniswap) from within
                                  the Samot Club website and earn a yield on their $AMOT tokens by doing so.
                                </p>
                              </div>
                            </div>
                          </Col>
                      ...
                      <Tab eventKey="launchpad-inner" title="Launchpad" className="inner-tab-content">
                        <Row>
                          <Col className="launchpad-header">
                            ...
                            <p>
                              We’re on a mission to introduce the most amazing artists and musicians in Latin
                              America to the NFT space. In partnership with WenMint, a US-based NFT
                              accelerator, and numerous top NFT communities, we can help transform your
                              artwork into non-fungible tokens (NFTs) that can be openly traded by millions of
                              collectors around the world. If you aren’t familiar with NFTs, don’t worry!
                              We’ll mentor you. Below is a list of the many services we provide as part of our
                              launchpad. If you’re interested with talking to someone on our team, click the
                              Apply button below and tell us about your work. We look forward to hearing from
                              you!
                            </p>
                          </Col>
                        </Row>
                        <Row>
                          <Col sm={4} className="launchpad-service">
                            ...
                            <p>
                              All projects accepted into the Samot LaunchPad will have a dedicated collection
                              page from which collectors can learn about the artist, artwork, and mint NFTs.
                            </p>
                          </Col>
                          <Col sm={4} className="launchpad-service">
                            <FaImages />
                            <h3>Art Curation</h3>
                            <p>
                              We will help convert your artwork into digital form that can be used to launch
                              your collection. This includes working through rarity and other features.
                            </p>
                          </Col>
                          <Col sm={4} className="launchpad-service">
                            <FaStar />
                            ...
                            <p>
                              We will create and deploy the smart contract, minting form, and metadata API
                              required to make your collection available on OpenSea and other marketplaces.
                            </p>
                          </Col>
                          <Col sm={4} className="launchpad-service">
                            <FaDiscord />
                            <h3>Discord Setup</h3>
                            <p>
                              We will help create a Discord server for your collection that can be used to
                              communicate with your community and keep them informed.
                            </p>
                          </Col>
                          <Col sm={4} className="launchpad-service">
                            <FaRoad />
                            ...
                            <p>
                              We’ll help you create and document a vision for your collection and artwork that
                              will appeal to NFT collectors and help shape the future of your project.
                            </p>
                          </Col>
                          <Col sm={4} className="launchpad-service">
                            <FaHandshake />
                            ...
                            <p>
                              Launching an NFT collection requires some manpower, including marketing people,
                              community managers, and other who can keep your community engaged.
                            </p>
                          </Col>
                          <Col sm={4} className="launchpad-service">
                            <FaUsers />
                            ...
                            <p>
                              We’ll show you how to jumpstart your community through the use of giveaways,
                              invite contests, influencer marketing, and collabs with other artists and
                              communities.
                            </p>
                          </Col>
                          <Col sm={4} className="launchpad-service">
                            ...
                            <p>
                              We’ll help you develop a longer-term marketing strategy that leverages social
                              media, video, and numerous other channels to keep your community engaged.{' '}
                            </p>
                          </Col>
                          <Col sm={4} className="launchpad-service">
                            <FaEthereum />
                            ...
                            <p>
                              We will help create awareness of your art throughout the NFT space by promoting
                              your project within our community, to our partners, and on social media.
                            </p>
                          </Col>
                        </Row>
                        <div className="samot_bg">
                          <div className="apply-now">
                            <h1>Are You Ready?</h1>
                            <p>
                              Existing Samot NFT holders will have exclusive, pre-sale access to your
                              collection, after which the general public will be able to mint your artwork. We
                              charge a small % of the ETH you raise during your sale for providing launch
                              support. The remainder of the ETH raised goes directly to you and you also
                              receive ongoing royalty fees for secondary sales. Click Apply below to tell us
                              about your work and someone from our team will be in touch.
                            </p>
                            <Button
                              target="_blank"
                              href="https://samot.art/apply"
      ...
      {selected && (
        <Modal show={show} onHide={handleClose} centered size="lg" className="details-modal">
          <Modal.Header>
            <Modal.Title className="nft-number">
              <span className="label">token</span> {selected.token_id}
            </Modal.Title>
          ...
          <Modal.Body>
            <div className="nft-img">
              ...
              <Button
                variant="outline-light"
                size="lg"
                href={`https://opensea.io/assets/${selected.asset_contract.address}/${selected.token_id}`}
                target="new"
              >
                View On OpenSea
              </Button>
      ...
      {error && (
        <Modal show={showError} onHide={handleErrorClose} centered className="dark-modal">
          <Modal.Header>
            ...
            <div className="closeBtn" onClick={handleErrorClose}>
              <AiOutlineClose />
            </div>
          </Modal.Header>
          <Modal.Body>
